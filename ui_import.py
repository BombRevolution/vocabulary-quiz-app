...
import importer
from ui_theme import COLOR_BG, COLOR_TEXT, COLOR_PRIMARY, FONT_BODY, FONT_HEADING, FONT_SMALL
...
class ImportDialog(tk.Toplevel):
    def __init__(self, root, conn, on_close=None):
        ...
        tk.Label(rows, text="导入词库", font=FONT_HEADING, bg=COLOR_BG,
        ...
        self.file_label = tk.Label(rows, text="未选择文件", font=FONT_SMALL, bg=COLOR_BG,
                                   fg=COLOR_TEXT, anchor="w")
        self.file_label.grid(row=1, column=1, sticky="ew", columnspan=2)

        tk.Label(rows, text="工作表", font=FONT_BODY, bg=COLOR_BG, fg=COLOR_TEXT).grid(row=2, column=0, sticky="w", pady=4)
        ...
        tk.Label(rows, text="英文单词列", font=FONT_BODY, bg=COLOR_BG, fg=COLOR_TEXT).grid(row=3, column=0, sticky="w", pady=4)
        ...
        tk.Label(rows, text="中文释义列", font=FONT_BODY, bg=COLOR_BG, fg=COLOR_TEXT).grid(row=4, column=0, sticky="w", pady=4)
        ...
        tk.Label(rows, text="词库名称", font=FONT_BODY, bg=COLOR_BG, fg=COLOR_TEXT).grid(row=5, column=0, sticky="w", pady=4)
